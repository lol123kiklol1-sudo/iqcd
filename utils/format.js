...
let tokensConfig = {};
try {
  tokensConfig = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'config', 'tokens.json'), 'utf8'));
} catch (e) {
  tokensConfig = {};
}
...
function buildSubscriptionEmbed(sub) {
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: '📦 نوع البوت', value: BOT_TYPE_LABELS[sub.botType] || sub.botType, inline: true },
...
function buildInviteUrl(sub) {
  const botCfg = tokensConfig[sub.botType];
  if (!botCfg || !botCfg.clientId || botCfg.clientId.startsWith('ضع_')) return null;
  const params = new URLSearchParams({
    client_id: botCfg.clientId,
...
function buildSubscriptionListEmbed(subs) {
  ...
  for (const sub of subs) {
    ...
    embed.addFields({
      name: `${BOT_TYPE_LABELS[sub.botType] || sub.botType}#${sub.subId.slice(-4)}`,
...
function buildSubscriptionSelectMenu(subs) {
  const options = subs.slice(0, 25).map(sub => {
    const label = `${(BOT_TYPE_LABELS[sub.botType] || sub.botType).replace(/^[^\s]+\s/, '')}#${sub.subId.slice(-4)}`;
    const statusTag = sub.status !== 'active' ? ` [${STATUS_LABELS[sub.status]}]` : '';
    return {
      label: `${label}${statusTag}`.slice(0, 100),
      description: `يوقف - آيدي السيرفر: ${sub.targetServerId}`.slice(0, 100),
      value: sub.subId,
    };
  });
